...
class EnemyTracker:
    ...
    def get_positions(self):
        ...
        return self.get_x().copy(), self.get_y().copy()
...
class SkirmisherTank(Bot):
    ...
    def move_with_wall_avoidance(self):
        ...
        if self.get_x() < margin:
            self.turn_rate = 90
        elif self.get_x() > self.get_arena_width() - margin:
            self.turn_rate = -(90)
        elif self.get_y() < margin:
            self.turn_rate = 90
        elif self.get_y() > self.get_arena_height() - margin:
            self.turn_rate = -(90)
