...
class ChampionBot(BaseBot):
    ...
    def on_scanned_bot(self, event):
        ...
        # CRITICAL EVASION: Only run if VERY close to avoid movement conflicts
        if distance < 80 and self.energy > 20:
            # Calculate angle AWAY from enemy
            angle_to_enemy = self.calculate_angle(self.get_x(), self.get_y(), enemy_x, enemy_y)
            escape_angle = (angle_to_enemy + 180) % 360  # Opposite direction
            turn_amount = escape_angle - self.get_direction()
            # Normalize
            while turn_amount > 180:
                turn_amount -= 360
            while turn_amount < -180:
                turn_amount += 360
            if abs(turn_amount) > 10:  # Only turn if significant
                self.turn_right(turn_amount)
            self.forward(70)  # Full speed away!
        ...
        if self.enemy_energy is not None:
            ...
            if 0 < energy_drop <= 3:
                ...
                # Don't call emergency_dodge here - it conflicts with movement
    ...
    def choose_optimal_power(self, distance, velocity):
        ...
        max_power = 3 if self.energy > 40 else 2
    ...
    def simulate_shot(self, target_x, target_y, power):
        ...
        bullet_x, bullet_y = self.x, self.y
    ...
    def on_hit_by_bullet(self, event):
        ...
        print(f"💥 Hit! Energy: {self.energy:.0f}")
    ...
    def turn_gun_to(self, angle):
        ...
        gun_turn_amount = angle - self.get_gun_direction()
        # Normalize to -180 to 180
